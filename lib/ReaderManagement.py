...
class ReaderManagement():
    ...
    def NewLibraryCard(self, rdInfoList):
        ...
        if not result == []:
            ...
            a = f"INSERT INTO TB_Reader VALUES({self.rdID},'{self.rdName}','{self.rdSex}',{self.rdType},\
            '{self.rdDept}','{self.rdPhone}','{self.rdEmail}',GETDATE(),'{self.rdPhoto}','{self.rdStatus}',\
            {self.rdBorrowQty},'{self.rdPwd}',{self.rdAdminRoles})"
            Library.ExecNonQuery(f"INSERT INTO TB_Reader VALUES({self.rdID},'{self.rdName}','{self.rdSex}',\
            {self.rdType},'{self.rdDept}','{self.rdPhone}','{self.rdEmail}',GETDATE(),'{self.rdPhoto}',\
            '{self.rdStatus}',{self.rdBorrowQty},'{self.rdPwd}',{self.rdAdminRoles})")
            return '借书证添加成功'
