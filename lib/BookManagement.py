...
class BookManagement():
    ...
    def NewBookIn(self, bkInfoList):
        ...
        if QueryList == []:
            ...
            Library.ExecNonQuery(
                f"INSERT INTO TB_Book VALUES({self.bkID},'{self.bkCode}','{self.bkName}',\
                '{self.bkAuthor}','{self.bkPress}','{self.bkDatePress}','{self.bkISBN}',\
                '{self.bkCatalog}',{self.bkLanguage},{self.bkPages},{self.bkPrice},GETDATE(),\
                '{self.bkBrief}','{self.bkCover}','{self.bkStatus}')")
        else:
    ...
    def EditBookInfo(self, bkID, EditDict):
        ...
        Library = MSSQL('Library')
        Library.ExecNonQuery(
            "UPDATE TB_Book SET bkName = '新的测试名称' WHERE bkID = {self.bkID}")

    def BookDestroyOrSell(self, bkID, bkStatus):
        Library = MSSQL('Library')
        self.bkID = bkID
        self.bkStatus = bkStatus
        bkNowStatus = Library.ExecQuery(
            f"SELECT bkStatus FROM TB_Book WHERE bkID = {self.bkID}")
        bkNowStatus = bkNowStatus[0][0]
        # 借出，遗失
        if bkNowStatus == '在馆':
            Library.ExecNonQuery(
                f"UPDATE TB_Book SET bkStatus = '{self.bkStatus}' WHERE bkID = {self.bkID}")
            return '执行成功'
        else:
